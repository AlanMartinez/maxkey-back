...
| 6 | Mercado Pago sandbox credentials | MP Developer dashboard → sandbox application → copy the sandbox `Access Token` into `Payments__AccessToken` (local: `dotnet user-secrets set "Payments:AccessToken" "<token>" --project src/Maxkeys.Api`, never in a config file). Create a sandbox **test buyer** (Sellers/Buyers panel) for step 5's checkout approval. |
| 6a | Frontend base URL | Set `Frontend__BaseUrl` to the Nuxt origin (`http://localhost:3000` locally, the Vercel production URL in prod) — without it, `back_urls` is omitted from the preference and MP shows its own confirmation page instead of redirecting to `/checkout/result`. |
