...
| 8a | maxkeys-back | Fulfillment application (attach key, admin listing, delivery email + handler) | `feat/mvp-08a-fulfillment-application` | `feat/mvp-07b-outbox-processor` → `main` after PR7b | 4, 7b | 611 (actual) | 3 | `dotnet test tests/Maxkeys.Application.Tests --filter Fulfillment` | N/A (`RecordingEmailSender` fake) | delete `Fulfillment/*`, `OrderDeliveredHandler`, `BuyerDeliveryItem`, `BuyerOrderDelivered` template |
| 8b | maxkeys-back | Orders history (owner-scoped) + fulfillment spec carry-over | `feat/mvp-08b-orders-history` | `feat/mvp-08a-fulfillment-application` → `main` after PR8a | 8a | 222 (actual) | 3 | `dotnet test tests/Maxkeys.Application.Tests --filter Orders` | N/A | delete `Orders/GetMyOrder(s)`, `OrdersDtos`; revert fulfillment spec edit |
| 9a | maxkeys-back | API skeleton core (host, DI, errors, logging, health, catalog, CORS, Dockerfile) | `feat/mvp-09a-api-skeleton` | `feat/mvp-08b-orders-history` → `main` after PR8b | 5, 7 | 742 (actual) | 4 | `dotnet build && dotnet test tests/Maxkeys.Api.Tests --filter "Catalog\|Cors"` | `dotnet run --project src/Maxkeys.Api` with empty `Payments:AccessToken`, hit `/health` | delete `src/Maxkeys.Api`, `tests/Maxkeys.Api.Tests` (revert to PR8b state) |
| 9b | maxkeys-back | API checkout endpoints + credential-less payment gateway | `feat/mvp-09b-api-checkout` | `feat/mvp-09a-api-skeleton` → `main` after PR9a | 9a | ~290 (actual) | 4 | `dotnet test tests/Maxkeys.Api.Tests --filter Checkout` | `dotnet run --project src/Maxkeys.Api`, `POST /checkout/orders` with empty `Payments:AccessToken` → 503 | delete `CheckoutEndpoints.cs`, `NotConfiguredPaymentGateway.cs`; revert `CheckoutDtos.cs`/`GetOrderStatus.cs`/`DependencyInjection.cs` to PR9a state |
| 10a | maxkeys-back | API auth core (JwtSetup, JWKS/HS256, JwksKeyCache, Admin policy, OptionalBearerFilter) | `feat/mvp-10a-api-auth-core` | `feat/mvp-09b-api-checkout` → `main` after PR9b | 8, 9 | 280 (actual) | 4 | `dotnet build && dotnet test tests/Maxkeys.Api.Tests --filter "Catalog\|Cors\|Checkout"` | `dotnet run` — `/health/live` 200, no new endpoints yet | delete `src/Maxkeys.Api/Auth`, revert `Program.cs`/`CheckoutEndpoints.cs`/`appsettings.json`/`Maxkeys.Api.csproj` to PR9b state |
| 10b | maxkeys-back | `/me` and `/admin` fulfillment endpoints + Auth.Tests (both JWT modes) | `feat/mvp-10b-api-auth-endpoints` | `feat/mvp-10a-api-auth-core` → `main` after PR10a | 10a | 559 (actual) | 4 | `dotnet test tests/Maxkeys.Api.Tests --filter Auth` | `dotnet run` + call `/me/orders` with a real Supabase token (manual, pending task 10.0) | delete `Auth/*` test folder, `MeEndpoints.cs`, `AdminEndpoints.cs`; revert `Program.cs`/`DependencyInjection.cs` to PR10a state |
| 11 | maxkeys-back | Mercado Pago integration | `feat/mvp-11-mercadopago-integration` | `feat/mvp-10b-api-auth-endpoints` → `main` after PR10b | 6, 9 | 784 (actual) | 5 | `dotnet test tests/Maxkeys.Api.Tests --filter Webhooks` | MP sandbox notification via public tunnel (task 11.6) | revert DI to `NotConfiguredPaymentGateway`; delete `MercadoPago*`, `WebhookEndpoints` |
| 12 | maxkeys-back | Email + catalog seed | `feat/mvp-12-email-seed` | `feat/mvp-11-mercadopago-integration` → `main` after PR11 | 7, 9 | 544 (actual, generated migration + model snapshot delta excluded) | 5 | `dotnet test tests/Maxkeys.Application.Tests --filter Catalog` | `dotnet run -- --seed-catalog seed/catalog.json` against a dev DB | set `Email:Sender=Logging`; delete `SmtpEmailSender`, `CatalogSeeder` |
...
**PR8** — split at apply time into **PR8a** `feat/mvp-08a-fulfillment-application` (tasks 8.1-8.4, 8.7 fulfillment/outbox tests; base: PR7b → `main` after merge; depends on: PR4, PR7b; 611 lines) and **PR8b** `feat/mvp-08b-orders-history` (tasks 8.5, 8.6, 8.7 orders tests; base: PR8a → `main` after merge; depends on: PR8a; 222 lines) — actual 833 lines exceeded the 800-line review budget

- [x] 8.1 Add `src/Maxkeys.Application/Fulfillment/AttachKeyToOrderItem.cs`: load order+items+keys (`xmin`), `KeyCipher.Encrypt`, `Key.Create` + `Order.AttachKey`, insert `OutboxEvent(OrderDelivered)` in the same tx as the last-key `Delivered` transition (fulfillment spec `Key Attachment`, `All-or-Nothing Delivery Derivation`); catch `DbUpdateConcurrencyException`, reload, retry once, else 409 (spec `Concurrent double-attach on the same item`).
- [x] 8.2 Add `src/Maxkeys.Application/Fulfillment/ListOrdersAwaitingFulfillment.cs` (per-item assigned/required counts — spec `Admin Order Listing`).
- [x] 8.3 Add `src/Maxkeys.Application/Outbox/OrderDeliveredHandler.cs : IOutboxHandler` — load order+items+keys, assert `Delivered`, `KeyCipher.Decrypt` each key in memory only, send one buyer email with all keys grouped by item.
- [x] 8.4 Add buyer delivery email template to `EmailTemplates.cs`.
- [x] 8.5 Add `src/Maxkeys.Application/Orders/GetMyOrders.cs` (owner-only — orders-history spec `My Orders Listing`), `GetMyOrder.cs` (keys only when `Delivered` — spec `Order Detail With Conditional Key Reveal`; 404 on non-owner — spec `Ownership Enforcement`, ADR-14).
- [x] 8.6 **Spec carry-over** — reword `One-Time Delivery Email` in `specs/fulfillment/spec.md` to: "The system MUST send the delivery email containing all keys exactly once per `AwaitingFulfillment → Delivered` transition, emitted via the `OrderDelivered` outbox event inserted in the same transaction as that transition. Handler retries MAY duplicate the email if the send succeeds but marking the event `Processed` fails (at-least-once)." Keep the existing "Email sent once even under retry/re-evaluation" scenario unchanged.
- [x] 8.7 `tests/Maxkeys.Application.Tests/Fulfillment/AttachKeyToOrderItemTests.cs`: over-quantity attach → 409; concurrent parallel attaches → one 409 then success on retry; last key → `Delivered` + one `OrderDelivered` row. `tests/Maxkeys.Application.Tests/Outbox/OrderDeliveredHandlerTests.cs`: email contains all keys once; re-evaluating an already-sent transition sends no second email (ADR-04). `tests/Maxkeys.Application.Tests/Orders/GetMyOrdersTests.cs`: owner scoping, cross-user 404.
- Test: `dotnet test tests/Maxkeys.Application.Tests --filter "Fulfillment|Orders"` green.

## Phase 4: API

**PR9** — split at apply time into **PR9a** `feat/mvp-09a-api-skeleton` (tasks 9.1-9.5, 9.8-9.9, 9.10 catalog/CORS/404 tests; base: PR8b (`feat/mvp-08b-orders-history`) → `main` after merge; depends on: PR5, PR7b; 742 lines) and **PR9b** `feat/mvp-09b-api-checkout` (tasks 9.6-9.7, 9.10 checkout tests; base: PR9a → `main` after merge; depends on: PR9a; ~290 lines) — the first full-solution measurement came in at 1051 lines, well over the session's 800-line hard cap, once the very first API host (csproj, Program.cs, DI composition root, error handling, logging, health, catalog, checkout, CORS, Dockerfile, and a brand-new `Maxkeys.Api.Tests` project) was accounted for

- [x] 9.1 Add `src/Maxkeys.Api/Maxkeys.Api.csproj`, `Program.cs` (minimal API bootstrap, DI via `Infrastructure/DependencyInjection.cs`), `appsettings.json`, `appsettings.Development.json`. (PR9a)
- [x] 9.2 Add `src/Maxkeys.Api/Errors/ProblemDetailsExceptionHandler.cs`: `DomainException`→422, `DomainConflictException`→409, `PaymentGatewayException`→503, unhandled→500 (design §7 error table). `NotFoundException` is not thrown anywhere yet (every use case returns `null` for not-found, matching the existing PR5b/PR8b pattern); 404s are returned directly from the endpoint via `Results.Problem(statusCode: 404)`, not via the exception handler. (PR9a)
- [x] 9.3 Add `src/Maxkeys.Api/Logging/SerilogSetup.cs`, `CorrelationIdMiddleware.cs`, `SensitiveDataPolicy.cs` (masks `Key`, `AttachKeyRequest`, `OrderDetail`, plus the concrete `MyOrderDetail`/`AdminOrderSummary` types by name — design §8/§12). (PR9a)
- [x] 9.4 Add `src/Maxkeys.Api/Endpoints/HealthEndpoints.cs` (`/health` DB check, `/health/live` process only). (PR9a)
- [x] 9.5 Add `src/Maxkeys.Api/Endpoints/CatalogEndpoints.cs` (`GET /catalog/products`, `GET /catalog/products/{slug}`). (PR9a)
- [x] 9.6 Add `src/Maxkeys.Api/Endpoints/CheckoutEndpoints.cs` (`POST /checkout/orders`, `GET /checkout/orders/{id}/status`). **HTTP body model MUST NOT bind `UserId` — derive it from the optional bearer `sub` only; body is `{email, items:[{variantId, quantity}]}` (design 7, frontend `types/api.ts`).** `CheckoutStatusResult` extended with `TotalAmount`/`Currency` (additive) to match the design §7 status response shape exactly. (PR9b)
- [x] 9.7 Add `src/Maxkeys.Infrastructure/Payments/NotConfiguredPaymentGateway.cs` — registered as `IPaymentGateway` when `Payments:AccessToken` is empty; throws `PaymentGatewayException` → 503, so `POST /checkout/orders` builds and runs green without MP credentials. (PR9b)
- [x] 9.8 Add `Cors:AllowedOrigins` option (array, `src/Maxkeys.Api/Cors/CorsOptions.cs`) and wire `UseCors` in `Program.cs` — configuration-driven allowlist (e.g. `http://localhost:3000` in dev, the Vercel production origin in prod), no wildcard origin ever registered (design §10/§12, ADR-18 CORS consequence). (PR9a)
- [x] 9.9 Add `Dockerfile` (multi-stage `sdk:8.0` → `aspnet:8.0`, non-root, port 8080). (PR9a)
- [x] 9.10 `tests/Maxkeys.Api.Tests/`: catalog endpoint scenarios (active-only, platform filter, search), 404 Problem Details shape, CORS test — request from an origin not in `Cors:AllowedOrigins` receives no `Access-Control-Allow-Origin` header, an allowed origin does (PR9a, 6 tests); checkout endpoint scenarios (422 missing email, 422 unknown variant, 503 with no gateway configured, extra `userId` body field ignored, 404 unknown order status) plus a request-body-shape guard test (PR9b, 6 tests).
- Test (PR9a): `dotnet build && dotnet test tests/Maxkeys.Api.Tests --filter "Catalog|Cors"` green with no MP credentials configured — 6/6. Full solution: Domain 56/56, Application 51/51, Api.Tests 6/6, 0 build warnings. Smoke: `dotnet run --project src/Maxkeys.Api` (Development env, empty `Payments:AccessToken`) started cleanly; `/health/live` → 200; `/health` → 503 Problem Details (no local Postgres running, as expected).
- Test (PR9b): `dotnet test tests/Maxkeys.Api.Tests --filter Checkout` green — 6/6; full `Maxkeys.Api.Tests` 12/12 (adds 6 checkout tests on top of PR9a's 6). Full solution: Domain 56/56, Application 51/51, 0 build warnings. Smoke: `dotnet run --project src/Maxkeys.Api` (Development env, empty `Payments:AccessToken`) — `/health/live` → 200; `POST /checkout/orders` against a variant id that cannot resolve without a reachable Postgres → 500 Problem Details (no local dev DB running — expected; the 422/503/404 checkout paths are exercised against the real Testcontainers-backed test host instead, per the tests above).

**PR10** — split at apply time into **PR10a** `feat/mvp-10a-api-auth-core` (tasks 10.1-10.4, auth infrastructure only; base: PR9b (`feat/mvp-09b-api-checkout`) → `main` after merge; depends on: PR8, PR9; 280 lines) and **PR10b** `feat/mvp-10b-api-auth-endpoints` (tasks 10.5-10.6, `/me` + `/admin` endpoints and Auth.Tests; base: PR10a → `main` after merge; depends on: PR10a; 559 lines) — the full slice measured 839 changed lines against the 800-line session hard cap once HS256 + JWKS test coverage (two extra `WebApplicationFactory<Program>` fixtures, a fake JWKS handler, an RSA test key helper) was accounted for

- [x] 10.0 **BLOCKING — Confirm Supabase project JWT signing mode (asymmetric JWKS vs legacy HS256) in the Supabase dashboard (Auth → JWT keys) and set `Auth:Mode` accordingly before implementing the rest of this PR.** Pending user confirmation in the Supabase dashboard; code supports both modes, default is JWKS. **Resolved 2026-09-13**: the project's public `/auth/v1/.well-known/jwks.json` returns one `EC`/`ES256` signing key (asymmetric mode), so `Auth:Mode=Jwks` (the default) is correct; `JwksAuthTests` now covers an ES256-signed token.
- [x] 10.1 Add `src/Maxkeys.Api/Auth/AuthOptions.cs` (`Auth:Mode`, `Issuer`, `Audience`, `JwksUrl`, `Hs256Secret`, `AdminSubs`). (PR10a)
- [x] 10.2 Add `src/Maxkeys.Api/Auth/JwksKeyCache.cs` (cache 10 min, refetch on unknown `kid` once), `JwtSetup.cs` (JWKS or HS256 branch by `Auth:Mode`, `MapInboundClaims=false` — auth spec `Configurable JWT Validation Mode`). (PR10a)
- [x] 10.3 Add `src/Maxkeys.Api/Auth/AdminPolicy.cs` (`sub` in `Auth:AdminSubs`; empty allowlist → always deny — auth spec `Admin Authorization Policy`, fulfillment spec `Admin Authorization`). (PR10a)
- [x] 10.4 Add `src/Maxkeys.Api/Auth/OptionalBearerFilter.cs` (present-but-invalid bearer on `POST /checkout/orders` → 401, not a silent guest order — design §6e). (PR10a)
- [x] 10.5 Add `src/Maxkeys.Api/Endpoints/MeEndpoints.cs` (`GET /me/orders`, `GET /me/orders/{id}`), `AdminEndpoints.cs` (`GET /admin/orders?status=AwaitingFulfillment`, `POST /admin/orders/{id}/items/{itemId}/keys`). (PR10b; also registers `GetMyOrders`/`GetMyOrder`/`ListOrdersAwaitingFulfillment`/`AttachKeyToOrderItem` in `DependencyInjection.AddUseCases`, per the PR9a DI-registration rule)
- [x] 10.6 `tests/Maxkeys.Api.Tests/Auth/`: HS256 mode with a locally signed token → 200; JWKS mode with a test key pair served from an in-process fake HTTP handler (no real network call, functionally equivalent to a real in-process endpoint) → 200; wrong `aud` → 401; expired/malformed → 401 (HS256 fixture), unknown-kid → 401 (JWKS fixture, auth spec `Invalid or expired token rejected`); `/admin/*` non-allowlisted → 403, empty allowlist → 403 (JWKS fixture, valid token); `/me/orders/{id}` other owner → 404 (orders-history spec `Cross-user access denied`); anonymous `/me/orders` → 401; anonymous checkout allowed; present-but-invalid bearer on checkout → 401 (`OptionalBearerFilter`). Requires Docker/Testcontainers per PR3; honors `TEST_POSTGRES_CONNECTION`. Disables xUnit cross-collection parallelization for this assembly (`AssemblyInfo.cs`) — three concurrent `WebApplicationFactory<Program>` hosts raced on Serilog's static bootstrap logger. (PR10b)
- Test (PR10a): `dotnet build && dotnet test tests/Maxkeys.Api.Tests --filter "Catalog|Cors|Checkout"` green — 12/12 (no regressions from PR9a/PR9b). Full solution: Domain 56/56, Application 51/51, 0 build warnings.
- Test (PR10b): `dotnet test tests/Maxkeys.Api.Tests --filter Auth` green — 13/13; full `Maxkeys.Api.Tests` 25/25. Full solution: Domain 56/56, Application 51/51, `dotnet build Maxkeys.sln` 0 warnings. Runtime harness: `dotnet run --project src/Maxkeys.Api` (Development env) — `/health/live` → 200, `/health` → 503 (no local Postgres, expected); anonymous `GET /me/orders` → 401; anonymous `GET /admin/orders` → 401; `POST /checkout/orders` with `Authorization: Bearer garbage` → 401. A real Supabase-token call to `/me/orders` remains pending task 10.0.
...
**PR11** — `feat/mvp-11-mercadopago-integration` — base: PR10b → `main` after merge — depends on: PR6, PR9 — 784 lines (actual)

- [x] 11.1 Add `src/Maxkeys.Infrastructure/Payments/MercadoPagoOptions.cs` (`Payments:AccessToken`, `WebhookSecret`, `WebhookEnabled`, `NotificationUrl`).
- [x] 11.2 Add `src/Maxkeys.Infrastructure/Payments/MercadoPagoSignatureValidator.cs`: HMAC-SHA256 over `id:<data.id>;request-id:<x-request-id>;ts:<ts>;`, hex-compare via `CryptographicOperations.FixedTimeEquals` (payments-webhook spec `Signature Validation Before Any I/O`).
- [x] 11.3 Add `src/Maxkeys.Infrastructure/Payments/MercadoPagoGateway.cs : IPaymentGateway` — typed `HttpClient`, `POST /checkout/preferences`, `GET /v1/payments/{id}` (ADR-10); DI selects this over `NotConfiguredPaymentGateway` when `Payments:AccessToken` is non-empty, decided **lazily** at resolution time (see Deviations) rather than at DI-registration time.
- [x] 11.4 Add `src/Maxkeys.Api/Endpoints/WebhookEndpoints.cs` (`POST /webhooks/mercadopago`): `WebhookEnabled=false` → 503 before any other check (spec `Kill Switch`); else validate signature → 401 with zero I/O on failure; else `ProcessPaymentNotification.Execute`.
- [x] 11.5 `tests/Maxkeys.Api.Tests/Webhooks/`: signature validator vectors — valid, tampered `v1`, missing header → 401 (spec `Invalid signature rejected`); `WebhookEnabled=false` → 503 with no state change; exact replay of the same `x-request-id` → 2xx, no state change (spec `Exact replay is a no-op`).
- [ ] 11.6 **Verify the `x-signature` manifest (segment order, lowercase `data.id`) against a real Mercado Pago sandbox notification** before merging; adjust `MercadoPagoSignatureValidator` if the sandbox payload differs from the documented manifest. Pending manual sandbox verification by the user; validator implements the documented manifest `id:<data.id>;request-id:<x-request-id>;ts:<ts>;`. **Desk-checked 2026-09-13 against the official Mercado Pago webhook docs**: template `id:[data.id_url];request-id:[x-request-id_header];ts:[ts_header];`, `data.id` lowercased, HMAC-SHA256 hex, matches `MercadoPagoSignatureValidator`. Deliberate difference: the docs say to omit a missing `data.id`/`x-request-id` segment; this API rejects such requests with 401 per the payments-webhook spec. Only the live sandbox round-trip remains (run with 18a.5).
- Test: `dotnet test tests/Maxkeys.Api.Tests --filter Webhooks` green — 9/9 (5 signature-validator unit tests + 2 endpoint tests on `WebhooksApiTestFixture` + 1 kill-switch test on `WebhookDisabledApiTestFixture` + 1 additional signature-header endpoint test). Full `Maxkeys.Api.Tests` 34/34 (25 pre-existing + 9 new, zero regressions). Full solution: Domain 56/56, Application 51/51, `dotnet build Maxkeys.sln` 0 warnings. Runtime harness: `dotnet run --project src/Maxkeys.Api` (Development env, `Payments:AccessToken` empty from `appsettings.json`) — `/health/live` → 200, `/health` → 503 (no local Postgres, expected); anonymous `/me/orders` → 401; anonymous `/admin/orders` → 401; `POST /checkout/orders` with `Authorization: Bearer garbage` → 401; `POST /webhooks/mercadopago` with no `x-signature` header → 401; a second run with `Payments__WebhookEnabled=false` (env var override) → `POST /webhooks/mercadopago` → 503, confirming the kill switch against the real host, not just the test host.

**PR12** — `feat/mvp-12-email-seed` — base: PR11 → `main` after merge — depends on: PR7a, PR9 — 544 lines (actual, generated migration + model snapshot delta excluded)

- [x] 12.0 Add `Product.Description` (Domain property + EF config `text` + additive migration `AddProductDescription`) and map it in `CatalogDtos`; the proposal ERD and the frontend `types/api.ts` `ProductDetail.description` require it (gap surfaced in PR5a2; `ProductDetail.Description` is currently always `""`). Also adds `Product.UpdateCatalogInfo` (used by the seeder, task 12.3) — design section 4.1 said "none" for `Product` transition methods, written before the seeder's upsert requirement (ADR-12) existed.
- [x] 12.1 Extend `src/Maxkeys.Application/Notifications/EmailOptions.cs` (already added in PR7a with `Sender`/`From`/`OperatorTo`) with `Smtp:Host/Port/UseStartTls/User/Password`. Kept `OperatorTo` in code (already shipped since PR7a) and reconciled design section 10's config matrix to `OperatorTo` instead of `OperatorAddress`, which never existed in code.
- [x] 12.2 Add `src/Maxkeys.Infrastructure/Email/SmtpEmailSender.cs : IEmailSender` (MailKit 4.17.0 — 4.8.0 as originally pinned triggered `NU1902` on a known moderate-severity advisory, `TreatWarningsAsErrors` failed the build; ADR-08). DI registers both `LoggingEmailSender` and `SmtpEmailSender` and selects between them via a scoped factory reading `EmailOptions.Sender` lazily through `IOptionsMonitor<EmailOptions>` at resolution time — the same pattern PR11 used for payment gateway selection, for the same reason (`WebApplicationFactory` config overrides are only visible after the host finishes building).
- [x] 12.3 Add `src/Maxkeys.Infrastructure/Persistence/CatalogSeeder.cs`, `seed/catalog.json` (4 realistic digital products — Steam Wallet Gift Card, PlayStation Plus Membership, FC 25 Points, Xbox Game Pass Ultimate — 10 ARS variants total); wire `Maxkeys.Api --seed-catalog <path>` in `Program.cs` (ADR-12, upsert by slug; variants matched within a product by the `(Region, Edition)` pair, the only stable business key a variant has). Also adds `ProductVariant.UpdateDetails` for the same reason as `Product.UpdateCatalogInfo` above.
- [x] 12.4 **Deviation, not implemented as literally written** — task 12.4 asked for a new `src/Maxkeys.Infrastructure/Storage/StorageOptions.cs` + `R2ImageUrlResolver.cs`. This requirement (catalog spec "Image URL Resolution") was already fully implemented in PR5a's `ImageUrlBuilder` (`src/Maxkeys.Application/Catalog/StorageOptions.cs`) — PR5a deliberately placed it in Application, not Infrastructure, because `GetCatalog`/`GetProductBySlug` consume it directly and Application must not reference Infrastructure (ADR-01). Adding a second, Infrastructure-side copy as this task describes would duplicate the seam and violate that layering rule, so no new files were added; `GetCatalog`/`GetProductBySlug` already return resolved absolute URLs via the existing `ImageUrlBuilder`.
- [x] 12.5 `tests/Maxkeys.Application.Tests/Catalog/CatalogQueriesTests.cs`: `GetCatalog` resolves an absolute image URL, not the raw key; `CatalogSeeder` upsert is idempotent on re-run (two writes against a temp seed file with a changed price, one verify — exactly one product row, price updated, no duplicate).
- Test: `dotnet test tests/Maxkeys.Application.Tests --filter Catalog` green — 11/11 (4 pre-existing `GetCatalogTests` + 4 `GetProductBySlugTests` [+1 new `Returns_description_from_the_product_entity`] + 2 new `CatalogQueriesTests`). Full `Maxkeys.Application.Tests` green — 54/54 (51 pre-existing + 3 new, zero regressions). Full `Maxkeys.Api.Tests` green — 34/34 (no regressions from the `Program.cs` `--seed-catalog` early-return branch). `Maxkeys.Domain.Tests` green — 56/56. `dotnet build Maxkeys.sln` 0 warnings. Migration applied cleanly: `dotnet ef database update` against a real Postgres container ran `InitialCreate` then `AddProductDescription` with no errors. Runtime harness: `dotnet run --project src/Maxkeys.Api -- --seed-catalog seed/catalog.json` against that same container — first run wrote 4 products / 10 variants (`SaveChanges completed ... 14 entities written`); second run against the identical file wrote 0 entities (`SaveChanges completed ... 0 entities written`) — confirmed via `psql` that `product_count=4`, `variant_count=10` after both runs, i.e. idempotent upsert, not the `WebApplicationFactory`/test-host harness.
