...
| 9 | maxkeys-back | API skeleton | `feat/mvp-09-api-skeleton` | `feat/mvp-08b-orders-history` → `main` after PR8b | 5, 7 | ~370 | 4 | `dotnet build && dotnet test tests/Maxkeys.Api.Tests --filter Catalog` | `dotnet run --project src/Maxkeys.Api` with empty `Payments:AccessToken`, hit `/health` | delete `src/Maxkeys.Api` (revert to PR8b state) |
