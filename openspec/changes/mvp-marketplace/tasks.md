...
| 10 | maxkeys-back | API auth | `feat/mvp-10-api-auth` | `feat/mvp-09b-api-checkout` → `main` after PR9b | 8, 9 | ~380 | 4 | `dotnet test tests/Maxkeys.Api.Tests --filter Auth` | `dotnet run` + call `/me/orders` with a real Supabase token | delete `Auth/*`, `Me/Admin` endpoints |
